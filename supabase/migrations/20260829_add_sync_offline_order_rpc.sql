-- ============================================================
-- RPC sync_offline_order — atomic commit transaksi POS offline
-- SECURITY DEFINER + SET search_path = public
-- Semua operasi dalam SATU transaksi: rollback total jika gagal.
-- Idempotency: retry dgn idempotency_key sama → NO-OP (already_exists).
-- ============================================================
...
BEGIN
  ...
  -- ============================================================
  -- 2. GUARD IDEMPOTENCY
  --    Ambil orderid ASLI dari DB (FOR UPDATE).
  -- ============================================================
  ...
  --    qty_before/qty_after dihitung dari stok SERVER aktual
  -- ============================================================
  FOR v_item IN SELECT * FROM jsonb_array_elements(v_items)
  LOOP
    v_pid      := (v_item->>'product_id')::bigint;
...
-- ============================================================
-- PERMISSION — aplikasi Juwita (anon/authenticated) memanggil via RPC
-- ============================================================
