-- ============================================================
-- KEBIJAKAN STOK POS: BOLEHKAN STOK NEGATIF
-- ============================================================
-- Non-destruktif: CREATE OR REPLACE FUNCTION public.sync_offline_order
-- (salinan dari 20260901_add_plu_weighed_product.sql dengan satu perubahan).
--
-- Perubahan:
--   Bagian 4 (VALIDASI STOK) TIDAK lagi memblokir transaksi saat stok kurang.
--   Validasi "IF v_stock < v_qty THEN RETURN insufficient_stock" DIHAPUS.
--   Stok 0 / kurang tetap boleh checkout → stok menjadi negatif.
--   Pengurangan stok tetap: v_qty_after := v_qty_before - v_qty (tanpa clamp ke 0).
--
-- Dipertahankan utuh:
--   - atomic transaction (satu fungsi), row locking (FOR UPDATE),
--   - guard idempotency (idempotency_key),
--   - rollback/error handling (EXCEPTION + status),
--   - order_items + stock_mutations (OUT) + validasi input item,
--   - pengecekan produk tidak ditemukan + lock produk di Bagian 4.
--
-- Jangan diedit di migration lama; cukup jalankan file ini sekali.
-- ============================================================
