-- ============================================================
-- SIMPAN KASIR PADA TRANSAKSI POS
-- ============================================================
-- 1. Tambah kolom orders.kasir (TEXT, nullable) untuk transaksi POS baru.
--    Transaksi lama tidak diubah (tetap NULL → rekap menampilkan
--    "(kasir tidak tersimpan)").
-- 2. Recreate RPC sync_offline_order agar menerima p_kasir dan
--    menyimpannya ke orders.kasir (kebijakan stok sama persis dengan
--    20260904_allow_negative_stock_pos.sql).
-- ============================================================

ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS kasir TEXT;
...
BEGIN
  ...
  -- ============================================================
  -- 4. LOCK produk + cek keberadaan (FOR UPDATE → serial antar kasir).
  --    TIDAK memblokir saat stok kurang: stok boleh menjadi negatif.
  -- ============================================================
  FOR v_item IN SELECT * FROM jsonb_array_elements(v_items)
  LOOP
    v_pid := (v_item->>'product_id')::bigint;
...
GRANT EXECUTE ON FUNCTION public.sync_offline_order(text, text, text, text, text, integer, text, text, text, text, text, integer, integer, integer, integer, text, jsonb) TO anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.sync_offline_order(text, text, text, text, text, integer, text, text, text, text, text, integer, integer, integer, integer, text, jsonb) FROM PUBLIC;
